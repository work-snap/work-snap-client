...
interface UserState {
  ...
  updateUserType: (type: "BUSINESS_OWNER" | "PART_TIME_WORKER") => Promise<void>;
  logout: () => Promise<void>;
    ...
    (set, get) => ({
      ...
      updateUser: (updates: Partial<User>) => {
        ...
        if (currentUser) {
          ...
          console.log("[ZUSTAND] 📝 updateUser - 사용자 정보 및 캐시 업데이트:", updatedUser);
        }
      },
      ...
      loadUser: async () => {
        try {
          ...
          console.log("[ZUSTAND] 🔍 토큰 확인:", token ? "토큰 존재" : "토큰 없음");
          console.log("[ZUSTAND] 🔍 캐시된 사용자 확인:", cachedUser ? "캐시된 사용자 존재" : "캐시된 사용자 없음");
          ...
          if (token && cachedUser) {
            try {
              ...
              console.log("[ZUSTAND] ⚡ 캐시된 사용자 정보 즉시 로드:", parsedUser);
              
              // 백그라운드에서 API 호출로 최신 정보 동기화
              api.setAuthToken(token);
              console.log("[ZUSTAND] 📡 백그라운드에서 토큰 검증 시작:", token?.substring(0, 20) + "...");
              ...
              try {
                ...
                console.log("[ZUSTAND] 📝 백그라운드 동기화 후 캐시 업데이트:", userData);
              } catch (backgroundError: any) {
                // 백그라운드 동기화 중 401 에러 발생 시 refreshToken 시도
                if (backgroundError?.response?.status === 401) {
                  console.log("[ZUSTAND] 🔄 백그라운드 동기화 중 401 에러 - RefreshToken으로 토큰 갱신 시도");
                  try {
                    const refreshResponse = await authApis.refreshToken();
                    const newAccessToken = refreshResponse.data.accessToken;
                    console.log("[ZUSTAND] ✅ 백그라운드에서 RefreshToken으로 새로운 토큰 발급 성공");
                    
                    // 새로운 토큰으로 다시 사용자 정보 조회
                    api.setAuthToken(newAccessToken);
                    const userData = await getUser();
                    console.log("[ZUSTAND] ✅ 새로운 토큰으로 백그라운드 동기화 성공:", userData);
                    
                    set({ user: userData });
                    localStorage.setItem("user", JSON.stringify(userData));
                  } catch (refreshError: any) {
                    console.error("[ZUSTAND] ❌ 백그라운드 RefreshToken 갱신 실패:", refreshError);
                    // RefreshToken 실패 시에도 캐시된 사용자 정보는 유지 (사용자 경험 향상)
                  }
                } else {
                  console.warn("[ZUSTAND] ⚠️ 백그라운드 동기화 실패, 캐시된 정보 유지:", backgroundError?.message);
                }
              }
              
            } catch (parseError) {
              console.warn("[ZUSTAND] ⚠️ 캐시된 사용자 정보 파싱 실패, API 호출로 대체");
              // 파싱 실패 시 API 호출로 대체
      ...
      fallbackToApiCall: async (token: string) => {
        try {
          api.setAuthToken(token);
          console.log("[ZUSTAND] 📡 토큰 검증 API 호출:", token?.substring(0, 20) + "...");
          const userData = await getUser();
          ...
          if (error?.response?.status === 401) {
            console.log("[ZUSTAND] 🔄 401 에러 발생 - RefreshToken으로 토큰 갱신 시도");
            try {
              const refreshResponse = await authApis.refreshToken();
              const newAccessToken = refreshResponse.data.accessToken;
              console.log("[ZUSTAND] ✅ RefreshToken으로 새로운 토큰 발급 성공");
              
              // 새로운 토큰으로 다시 사용자 정보 조회
              api.setAuthToken(newAccessToken);
              const userData = await getUser();
              console.log("[ZUSTAND] ✅ 새로운 토큰으로 사용자 정보 조회 성공:", userData);
              
              set({ user: userData, isLoading: false });
              localStorage.setItem("user", JSON.stringify(userData));
              
            } catch (refreshError: any) {
              console.error("[ZUSTAND] ❌ RefreshToken으로 토큰 갱신 실패:", refreshError);
      ...
      isBusinessVerified: () => {
        const user = get().user;
        return user?.businessVerificationStatus === "APPROVED" || 
               user?.businessVerificationStatus === "VERIFIED";
      },
      
      isBusinessPending: () => get().user?.businessVerificationStatus === "PENDING",
      isBusinessRejected: () => get().user?.businessVerificationStatus === "REJECTED",
