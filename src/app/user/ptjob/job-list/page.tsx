...
export default function PtJobPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col max-w-[430px] w-full mx-auto relative pb-[80px]">
      ...
      {/* 대기 메시지 (인증 안 된 경우) */}
      {!isVerified && (
        <div className="px-4 mt-4 flex flex-col items-center justify-center py-8">
      ...
      {/* 출근 카드 리스트 (인증 완료 시 자동 표시) */}
      {isVerified && (
        <div className="px-4 mt-4 flex flex-col gap-3">
          {workPlaces?.data.map((wp) => (
            <div
              key={wp.id}
              className="rounded-xl border border-gray2 p-4 flex items-center justify-between mb-3"
            >
              <div>
