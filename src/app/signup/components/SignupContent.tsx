...
export default function SignupContent() {
  ...
  // SSR 중에는 정적 UI만 렌더링
  if (!isMounted) {
    return <SignupStaticUI />;
  }

  return (
    <>
      <SignupHeader onBack={handleBack} onLogout={handleLogout} />
      <SignupForm
...
/**
 * 정적 UI (SSR 렌더링용)
 * - 클라이언트 로직 없이 순수 UI만 렌더링
 */
function SignupStaticUI() {
  return (
    <>
      {/* 상단 헤더 */}
      ...
      <div className="flex flex-col items-start px-4 pt-2 w-full">
        ...
        <div className="w-full flex flex-col gap-4">
          <button className="w-full bg-gray2 rounded-xl py-7 flex flex-col items-center justify-center">
            <UserTypeIcon type="worker" />
            <span className="mt-2 text-gray5 font-semibold">알바님</span>
          </button>
          <button className="w-full bg-gray2 rounded-xl py-7 flex flex-col items-center justify-center">
            <UserTypeIcon type="owner" />
 ...
 * 사용자 타입 아이콘 (재사용 가능)
 */
function UserTypeIcon({ type }: { type: "worker" | "owner" }) {
  if (type === "worker") {
    return (
      <svg
        xmlns="http://www.w3.org/2000/svg"
