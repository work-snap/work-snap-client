...
export default function AddWorkPage() {
  ...
  // 사용자의 사업장 목록 조회
  const { data: workplacesResponse, isLoading: workplacesLoading } =
    useGetMyWP();

  // WorkplaceDetail[]을 WorkplaceSummary[]로 변환
  const workplaces =
    workplacesResponse?.data?.map((workplace) => ({
  ...
  const handleTimeChange = useCallback(
    ({ startTime, endTime }: { startTime: string; endTime: string }) => {
      console.log(
        `[AddWorkPage] handleTimeChange: startTime=${startTime}, endTime=${endTime}`
      );
      setForm((prev) => ({
        ...prev,
        startTime,
  ...
  return (
    <div className="flex flex-col bg-white w-full min-h-0 h-dvh">
      ...
      <div className="overflow-y-auto p-4">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <WorkplaceDropdown
              ...
              onChange={(workplaceId) =>
                handleInputChange("workplaceId", workplaceId)
              }
              label="사업장을 선택하세요"
            />
            {workplacesLoading && (
          ...
          <div className="space-y-4">
            ...
            <TimePicker onChange={handleTimeChange} debug={true} />
          </div>
